...
- TheRouterFlowTask.APP_ONCREATE：(已废弃，预计在1.1.4版本删除)
...
- **TheRouterFlowTask.THEROUTER_INITIALIZATION**：当TheRouter初始化后，执行相关依赖任务，此任务不能设置异步执行(考虑使用场景比较小众，支持会复杂，不如直接手动初始化)。(1.1.2-rc6版本新增)    
